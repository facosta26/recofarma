...
def create_app():
    ...
    def process_video_data(video_data):
            predictor = dlib.shape_predictor("shape_predictor_68_face_landmarks.dat")
            frame = np.frombuffer(base64.b64decode(video_data), dtype=np.uint8)
    ...
    def handle_video_data(video_data):
        print("se envia imagen")
        processed_frame = process_video_data(video_data['imageData'])
        emit('processed_video_data', {'processedImageData': processed_frame})


    @socketio.on('prueba')
    def prueba():
        emit('procesamiento', {'procesamiento':'ejecutado'})
